...
- Truncate `quotes` / `opportunities` / `trades` / `post_goal_samples` / `goal_context_observe` / `livescore_observe` / `bridge/events` by timestamp; **unprocessed** bridge events are always kept
