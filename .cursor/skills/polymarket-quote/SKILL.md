...
| 策略 | 触发 | 门控 | 金额 | Rest |
|---|---|---|---|---|
| Pitch-gate | 已配对进球，同帧 DOM `in_play` ∧ AF `score_match` | 是 | `QUOTE_GOAL_MAX_USDC` | 仅 `QUOTE_REST_ENABLED=1`；ask 低于 0.30 不买不挂 |
| Locked sweep | 门控买时 token 在上一分已是 live WIN | 同一刀 | `QUOTE_LOCKED_SWEEP_USDC` | 同门控；ask 低于 0.30 跳过 |
| T+10 | 进球后默认 480s（8min）；AF live **必须等于触发那球的比分**，否则 skip | **AF live = 触发比分**（FT/ET 交给终场） | `QUOTE_T10_USDC`（FAK 与 0.99 GTC **各**一份） | 每已锁 WIN token 一笔；ask 低于 0.30 不买不挂 |
...
- **判定源默认读动画 DOM，不截图 / 不跑 OCR**：`QUOTE_GATE_SOURCE=dom`（默认）共用一台 Chromium，进行中已配对场预开 tracker 页；同场后续进球复用标签。每次采样 `evaluate` `.pop-box` 与 `.center-box` 的 **textContent**（仍要跑页面 JS；默认关掉没用的 WebGL/3D 球场）。无 JPEG、无 `QUOTE_GATE_REF_SCREENSHOT`。标签上限 `QUOTE_DOM_POOL_MAX`（默认 **4**，满则踢最久未用的空闲页）。预热 `QUOTE_DOM_WARM`（默认开）/`QUOTE_DOM_WARM_INTERVAL_S`（默认 10s）/`QUOTE_DOM_WARM_OPEN_TIMEOUT_S`（默认 3s）。开页等待动画时会穿插处理其它场的 DOM 读。`QUOTE_DOM_WEBGL=1` 才恢复 3D。Chromium 关掉 site isolation，并把 `--renderer-process-limit` 对齐标签上限，避免一个 tracker 页拆成多个渲染进程把 4G 机器打满。
- **`QUOTE_GATE_SOURCE=mqtt`**：不启动 Chromium。订阅纳米 `wss://trackermq.namitiyu.com/mqtt` 的 `live/m1/{nami_id}`，把 `vc.code` 合成同一套 `pop_box` / `center_box` 给 `judge_dom()`。HTTP `variable_detail` 只在订阅和预热里打（超时默认 2s），不挡 5s 门控采样。连不上会丢掉 client 以便重试；断线复用同一个循环，不新开。MQTT 挂了门控 `unavailable`，**不会**回退 DOM。`ocr` 已废弃，其它值按 `dom`。
- **防僵死**：判定要求 `.center-box` 时钟相对上一次读数有推进；时钟没走 → `unclear`（`stale_page`），不下单。
- **页面是纳米 tracker**：`animation_live` URL 带 `nami_id`。DOM 路径打开 `tracker.namitiyu.com` 读 overlay；MQTT 路径只订阅同一 id 的推送。都不做球位观察。
- Pitch-gate 限价 rest：需 **`QUOTE_REST_ENABLED=1`** → 目标 @**0.995**，但 **不信** Gamma/book 的 `0.001` 元数据；CLOB 足球最小 tick 是 **0.01**，rest 一律按 0.01 **向下收到 0.99**。**`QUOTE_REST_USDC`（默认 $5）** / **`GTC` 一直挂着**（回撤、终场、手取消才撤；`QUOTE_REST_EXPIRE_S>0` 才改回 GTD）。挂单金额 **min(固定, 钱包 USDC)**，余额不够就按余额挂。门控 rest **不受** `QUOTE_MAX_OPEN_USDC` 限制。没有卖盘（一边倒买盘）也挂，等砸盘。可见 ask 低于 **`QUOTE_GATE_MIN_BUY_PRICE`（默认 0.30）** 或 ≤0.01 则不挂。FAK / misprice 上限 **ask≤0.995**（fee 后 `min_net≈0.00475`）。**进球作废仍 WIN：** 比分变化后若该 token 在 **上一分**（`prev`）已经是 live WIN（当下这球不算也锁死），pitch-gate **FAK 吃光** ask 在 **0.30–0.995** 的剩余卖盘，**不走** `QUOTE_GOAL_MAX_USDC` $50。金额顶 **`QUOTE_LOCKED_SWEEP_USDC`（默认 $1000）**，仍受 `QUOTE_MAX_OPEN_USDC` 剩余额度。开关 **`QUOTE_LOCKED_SWEEP`**（默认开；`0` 或金额 `0` 关闭）。回撤后若仓位在新比分仍是 WIN（如 1-1→1-0 的主队 0.5 大球），**不平仓**。终场已锁定 WIN 且 ask≤0.01：**仍 FAK**（`QUOTE_FT_DUST_FAK` 默认开），金额 **`QUOTE_FT_DUST_USDC`（默认 $100）**，独立于 `QUOTE_FT_MAX_USDC`，仍受 `QUOTE_MAX_OPEN_USDC` 剩余额度限制；max_price=0.01，不按 0.001 墙走单。
...
- **半场盘**：1H/2H O/U、BTTS、**Spread** 只用对应半场比分（2H = 全场−半场）。下半场懂球帝 `hts` 会写成当前比分；match-bridge 在 **1H/HT 冻结 HT**（`data/bridge/half_scores.json`），2H 事件带 `period` 且半场用冻结分。询价再取 ≤ 当前且总进球最少的候选，**等于当前比分的 hts 在 2H 直接丢掉**。缺半场比分的半场盘跳过。ask≤0.01 的「WIN」不挂 0.99 rest。
...
2. Bridge events in `data/bridge/events.jsonl`:
  ...
  - `score_change` reversal → first cancel/block the undone goal, then cancel rest + pitch-gate **and that goal's T+10** (queued or in-flight AF live; earlier standing T+10 on the same match stay). If lots are open, 5s AF trail → flatten on first AF `score_match` vs post-reverse score **only lots that are no longer WIN** (e.g. 1-1→1-0 keeps home O/U 0.5), then stop the trail.
...
| `--min-buy-price` | 0.6 | leftover non-gate path only; pitch-gate / T+10 use `QUOTE_GATE_MIN_BUY_PRICE` (0.30); **FT skips** |
