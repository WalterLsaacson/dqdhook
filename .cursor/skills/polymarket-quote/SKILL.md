...
4. **AF referee (gate default)**: on goal-up, **async** poll AF events (**every 2s from 2s until timeout**, default 90s). Concurrent confirms share one AF client paced by `QUOTE_AF_MIN_INTERVAL_S` (default 1s). `cache_only`; gate uses `wait_cache=False` (miss → skip). Confirm → quote+trade on AF score; timeout/miss → ignore (no buy, no flatten). DQD score reversals cancel pending AF jobs and open the 5s × 6 Odds rollback arbitration window; DQD alone never flattens. Aggressive `--af-postcheck-trade`: buy on DQD immediately, flatten on AF timeout. Live quoting uses **one** CLOB `/books` POST then totals/BTTS before exact.
...
| Odds confirmation | `data/pm-quote/book_context_observe.jsonl` | AF-confirmed goals at 0/3/…/90s: Odds-API.io score + Bet365 gate + 1xbet observe-only, A/B/C reason, change fingerprint and upgrade action |
