...
8. On misprice, executor plans fills (`--take-depth top|walk`) and writes `trades.jsonl`; `--live` or per-signal `--goals-mode`/`--ft-mode` posts market **FAK** via `py-clob-client-v2`. Live `score_change` skips FAK and rest when `best_bid` is missing or **< 0.85**. **Just-crossed A/B remainder** rests **GTD** bids at **0.99 then 0.98** toward `$10`/`$20`. **Two-goal-cushion WIN** (still locked after −1 on either side) caps **$10** at **0.99 GTC** (no wall-clock expire). C never rests. DQD reversal / FT / a token that is no longer WIN **cancels those bids immediately** (does not wait for Odds flatten).
...
| `QUOTE_TEST_SIZING` | off | Test profile: B/cushion `$1`, A `$2`; prod defaults remain `$10`/`$20` |
| Live bid floor | 0.85 | `score_change` buy_win skips FAK+rest if `best_bid` missing or below |
| Rest GTD (just-crossed A/B) | 0.99 + 0.98 | After FAK / empty asks, remainder toward `$10`/`$20`; `QUOTE_REST_EXPIRE_S` default 3600 (1h; 0=GTC). Cancel on DQD reversal immediately |
| Rest GTC (2-goal cushion WIN) | 0.99 only | Cap `$10`; `expire_s=0` GTC (CLOB/market expiry). Just-crossed keeps the GTD ladder |
| Flatten FAK floor | entry × 80% | Max 20% loss vs entry (tick-floored; unknown entry `0.5`) |
...
| DQD stream observe | `data/pm-quote/dqd_stream_observe.jsonl` | DQD goal t0/+10…+50s screenshot metadata |
| DQD stream frames | `data/pm-quote/dqd_stream_frames/` | JPEG frames from page/video capture |
| Pitch-state judge | `data/pm-quote/pitch_state_judge.jsonl` | One resume-play verdict per screenshot frame |
| Pitch-state frame sidecars | `data/pm-quote/dqd_stream_frames/<match>/<event>/*.json` | Per-frame judgment next to each JPEG |
