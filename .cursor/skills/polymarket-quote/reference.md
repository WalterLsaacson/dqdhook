...
- With AF referee on (**gate default**), goal-ups **wait for AF confirm** before trading (no preconfirm CLOB quote). AF default cadence is **every 2s from 2s until 90s timeout**, with a **shared** `QUOTE_AF_MIN_INTERVAL_S` (default 1s) across confirm workers; `--af-poll` still overrides the schedule with a fixed interval. Gate submits with `wait_cache=False` (fixture-cache miss → skip). Confirm → quote/trade. DQD reversal **cancels** pending AF jobs, but does not directly flatten. It starts six Odds-API.io samples at **5/10/15/20/25/30s**; the first hard-verified Odds score decrease or reappearance of a Bet365 Correct Score / main Totals offer that was impossible at the pre-reversal score emits one flatten decision and cancels the remaining samples. No corroboration within 30s means no flatten. AF timeout / miss → ignore the goal (no flatten). Aggressive `--af-postcheck-trade`: buy on DQD score immediately; AF confirm marks lots `af_status=confirmed` (hold); AF timeout flattens `af_pending` lots (`reason=af_confirm_timeout`). Event identity is semantic (`mid|prev→curr`, no wall-clock `ts`).
...
- An Odds-confirmed reversal FAK-flattens only affected open `buy_win` lots whose **entry_score** is strictly higher than the corrected score. It does not quote/buy the corrected DQD score. Unaffected lots stay open.
- Live flatten sells floored shares (**2 decimal** maker precision) with a **99% haircut**. Before pricing the FAK floor, ledger ``shares``/``usdc`` are reconciled to the live conditional balance (raise shares on better fills; scale ``usdc`` on residuals so VWAP holds). FAK `min_price` = **entry × (1 − 20%)** (tick-floored; unknown entry falls back to `0.5`) — no panic dump at `0.01` after false goals; unfilled residual stays `pending_flatten` for later retries. Before the initial sell: refresh conditional allowance + cancel open orders for that token (frees `sum_of_matched_orders` locks). On CLOB `not enough balance` → cancel again, size by **gate free only while live bal still matches the gate bag** (else size from live bal alone), same entry−20% floor. If still locked, **keep `pending_flatten` for the next tick** (no inline sleep on the watch path). An accepted asynchronous sell persists its `orderID`, submitted balance/status/time, and is reconciled at a throttled cadence; ordinary watch ticks neither write retry rows nor asset-cancel/repost it. Zero/dust balance writes one `flatten_settled`; a proven failed order or stable real residual may produce one replacement, while `MATCHED` with an unchanged/lagging balance never triggers a duplicate sell. Dust close only when live balance itself is `< 0.01`.
...
- **A/B rest bids**: live `score_change` skips FAK and rest when `best_bid` is missing or **< 0.85**. After FAK (or when a WIN leg has no ask), just-crossed remaining size toward `$10`/`$20` is posted as **GTD** limit buys at **0.99 then 0.98** (`QUOTE_REST_PRICES`, `QUOTE_REST_EXPIRE_S` default **3600s / 1h**; `0` = GTC). A two-goal-cushion WIN (still locked after −1 on either side) instead caps **$10** at **0.99 GTC** (no wall-clock expire). Working rest USDC counts toward the grade target and `QUOTE_MAX_OPEN_USDC`. A DQD score drop **cancels these orders immediately** (before Odds arbitration / flatten) and blocks new rests until a DQD restore. FT and tokens that are no longer WIN also cancel. Watch reconciles partial fills into the open lot. C never rests.
