...
With `ODDS_API_IO_KEY`, every AF-confirmed goal starts 13 samples at `0/5/…/60s`. Each sample fetches only Odds-API.io: `/odds` for Bet365 (gate) plus DraftKings (observe-only) and `/events/{id}` score/clock in parallel. DraftKings is written into `book_context_observe.jsonl` / `book_context_raw/` with `observe_only=true` and does not affect A/B/C. OddsPapi and The Odds API remain disabled (legacy parsers may remain but are unreachable from the active source config).
