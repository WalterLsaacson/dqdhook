...
With `ODDS_API_IO_KEY`, every AF-confirmed goal starts 31 samples at `0/3/…/90s`. Each sample fetches only Odds-API.io: `/odds/multi` for Bet365 (gate) plus 1xbet (observe-only) and `/events/{id}` score/clock in parallel (concurrent odds pulls coalesce, up to 10 events per request). 1xbet is written into `book_context_observe.jsonl` / `book_context_raw/` with `observe_only=true` and the same CS/Totals core-clean fields (`observe_books`); it does not affect A/B/C. Missing 1xbet does not veto. OddsPapi and The Odds API remain disabled (legacy parsers may remain but are unreachable from the active source config).
