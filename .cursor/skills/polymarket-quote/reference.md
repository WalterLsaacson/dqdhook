...
With `ODDS_API_IO_KEY`, every AF-confirmed goal starts 61 samples at `0/1/…/60s`. Each sample fetches only Odds-API.io: `/odds/multi` for Bet365 (gate) plus Sbobet (observe-only) and `/events/{id}` score/clock in parallel (concurrent odds pulls coalesce, up to 10 events per request). Sbobet is written into `book_context_observe.jsonl` / `book_context_raw/` with `observe_only=true` and does not affect A/B/C. OddsPapi and The Odds API remain disabled (legacy parsers may remain but are unreachable from the active source config).

Grades are cumulative per Polymarket token/opportunity for **A/B live sizing**: C is **record-only** (dry-run into `trades.jsonl` at a notional `$1` plan, no CLOB post, no open lot); B=`$2` when Bet365 is open, at least one full-match score-sensitive market is inspectable, and there are no offers already impossible at the target score; A=`$3` when that same Bet365 snapshot is clean **and** the Odds-API.io provider score exactly equals AF/DQD. A matching provider score with Bet365 missing, only Spread/ML and no inspectable score-sensitive markets, or any already-impossible offer stays C. Book labels such as `Bet365 (no latency)` are treated as Bet365. A/B are allowed only after that poll's `/events/{id}` body revalidates both teams, kickoff within ±12h, a non-terminal status and home/away direction. Provider-reversed fixtures are normalized to the DQD score frame, while impossible-market checks remain in the provider frame. Missing identity forces C; an identity/time/status mismatch clears the cached id for remapping on the next poll. Examples of impossible offers include a Correct Score below either observed team score, an Under line below goals already scored, BTTS No after both teams scored, or Clean Sheet Yes after the opponent scored.

Because C never registers exposure, B sizes the full `$2` and a direct A sizes the full `$3`; B→A adds at most `$1`. Existing open-budget, share, price and depth guards still apply. Live matched buys are accounted from CLOB `makingAmount` (USDC) and `takingAmount` (shares), not the requested FAK amount. If an A/B quote errors or only partially fills, the same upgrade is retried every 1–5s while its observer generation is current; success, DQD reversal or a replacement goal generation stops it. A strict grade increase creates the upgrade edge; same-grade market/score changes remain research rows.
