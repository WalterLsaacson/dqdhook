...
`buy_win` leftover floor: skip (still append `trades.jsonl` with `skip_reason=buy_price_below_min=…`) when `best_ask < --min-buy-price` (default **0.6**; **0** = off). Env: `QUOTE_MIN_BUY_PRICE`. **FT skips this floor.** Pitch-gate / T+10 use `QUOTE_GATE_MIN_BUY_PRICE` (default **0.30**; **0** = off) for both FAK and 0.99 rest.
...
9. **Rest fallback**: when **`QUOTE_REST_ENABLED=1`**, if pitch-gate WIN has no FAK fill, post a limit bid snapped onto the **CLOB-legal tick** (`GTC` by default — stays until DQD reversal, FT, or manual cancel), including one-sided bid books with no ask. Skip rest when visible ask is ≤0.01 or below `QUOTE_GATE_MIN_BUY_PRICE` (default **0.30**). Soccer CLOB minimum is **0.01** even when metadata says `0.001` — rest **clamps tick to 0.01** and **0.995 → 0.99**. Set **`QUOTE_REST_EXPIRE_S>0`** to use `GTD` instead. Size is **`min(QUOTE_REST_USDC, wallet USDC)`** (default rest **$5**) so the bid clears the CLOB 5-share floor when the wallet allows. Pitch-gate rest is **not** clipped by `QUOTE_MAX_OPEN_USDC`.
10. **T+10 rescan**: each paired goal-up schedules one job (`data/pm-quote/t10_pending.json`) for **`QUOTE_T10_DELAY_S`** (default 480s), independent of whether pitch-gate bought. A DQD reversal **cancels** the undone goal's queued / in-flight T+10 (`t10_skip_reversal`; stem match, `ts` ≤ reverse `ts`) and records `blocked_until` so a job already `pop_due`'d cannot submit. A later re-award of the same transition with a newer ts can schedule a new job. At fire, poll AF **live** goals (`QUOTE_T10_AF_TIMEOUT_S`, default 90s) **only while AF status is still in play**. Quote only when that live tally **exactly matches the triggering goal's score**; a later goal skips (`t10_skip_score_mismatch`) and does **not** rewrite-and-buy. If AF is FT/ET/PEN / `regulation_ready`, or a `match_finished` AF confirm is already pending, skip (`t10_skip_af_finished` / `t10_skip_ft_pending`) and let the FT path quote `score.fulltime`. Dongqiudi `prev_scores` is skeleton only (sides / halves, re-oriented onto Polymarket). Cache miss / timeout → skip, no DQD fallback. Then the same fee-aware `buy_win` path (`QUOTE_GATE_MIN_BUY_PRICE` 0.30; **no** locked sweep; **no** flatten on this quote). **`QUOTE_T10_USDC`** sizes **both** FAK and a 0.99 GTC rest (rest posts even when `QUOTE_REST_ENABLED=0`). Rest is a **second** T10-sized bid per **locked WIN token** (totals Over / BTTS / exact No already dead), not leftover after FAK, and is **not** clipped by `QUOTE_MAX_OPEN_USDC`. Unset / `0` / `QUOTE_T10=0` disables. Skip if AF has already confirmed FT; cancel pending jobs after AF regulation confirm (not on DQD `match_finished` alone). Jobs more than **`QUOTE_T10_MAX_LATE_S`** (default 900s) late are dropped.
...
**Animation source (纳米 tracker):** `QUOTE_GATE_SOURCE=dom` (default) uses one shared Chromium; in-play paired fixtures are pre-opened from `animation_live` (`https://tracker.namitiyu.com/zh/football?profile=…&id=<nami_id>`). A goal **evaluates** `.pop-box` / `.center-box` **text** on that tab (same match reuses it). Chromium still runs page JS (a renderer per tab is required); the unused WebGL/3D pitch is **off** by default (`QUOTE_DOM_WEBGL=1` to restore). Cap `QUOTE_DOM_POOL_MAX` (default **4**). Warming is on by default (`QUOTE_DOM_WARM`, interval `QUOTE_DOM_WARM_INTERVAL_S` default 10s, open timeout `QUOTE_DOM_WARM_OPEN_TIMEOUT_S` default 3s). Chromium launches with site isolation off and `--renderer-process-limit` matching the tab cap so one tracker page does not become several ~200MB renderer processes. While an open waits for the animation root, pending DOM reads are drained so a goal sample is not stuck behind warm. No ball-xy, no `page.screenshot`. Fixtures with no `animation_live` fall back to the DQD page iframe. Missing animation → timeout, no buy.

`QUOTE_GATE_SOURCE=mqtt` skips Chromium. One process-wide client on `wss://trackermq.namitiyu.com/mqtt` subscribes `live/m1/{nami_id}` (HTTP `variable_detail` on subscribe and the warm loop, not on each 5s gate tick). Overlay is synthesized from `vc.code` into the same `pop_box` / `center_box` / `marks` dict `judge_dom()` already consumes. MQTT down → gate `unavailable` (no silent DOM fallback). Connect timeout abandons the client so the next start/warm can retry; disconnect waits for the same client to reconnect instead of opening a second loop. Stale: no `10101` for `QUOTE_MQTT_STALE_S` (default 8s) while in-play freezes the clock so `stale_page` still fires. `ocr` is rejected.

Smoke: `python3 .cursor/skills/polymarket-quote/scripts/smoke_pitch_gate.py`, `.../smoke_locked_sweep.py`, `.../smoke_t10_scan.py`, `.../smoke_half_settle.py`, `.../smoke_pitch_gate_dom.py`, `.../smoke_nami_mqtt.py`, `.../smoke_dom_page_pool.py`, `.../smoke_af_observe.py`, `.../smoke_book_context_observe.py`, `.../smoke_prematch_odds.py`, `python3 .cursor/skills/dongqiudi-match/scripts/smoke_dqd_live.py`.

**Gate source: `dom` or `mqtt`**

`gate_source()` reads `QUOTE_GATE_SOURCE` (`mqtt` or else `dom`). `judge_dom()` applies `IN_PLAY_TOKENS` / `STOPPED_TOKEN_MAP` to overlay text. VAR veto and score matching are unchanged. Frozen clock → `stale_page`. Clock must not be parsed as a scoreline.
