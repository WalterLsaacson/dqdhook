...
With `ODDS_API_IO_KEY`, every AF-confirmed goal starts 61 samples at `0/1/…/60s`. Each sample fetches only Odds-API.io: `/odds/multi` for Bet365 and `/events/{id}` score/clock in parallel (concurrent odds pulls coalesce, up to 10 events per request). OddsPapi and The Odds API remain disabled (legacy parsers may remain but are unreachable from the active source config).
